...
import { GetPreviewUrlParams, GetVodUrlParams, VideoUrl } from "../model/waste-regulation/video-model";
...
export class SRServersRequestService {
    ...
    async PreviewUrls(params: GetPreviewUrlParams) {
        ...
        const response = await this.requestService.post<GetPreviewUrlParams, Response<VideoUrl>>(this.url.preview(), params);
    ...
    async VodUrls(params: GetVodUrlParams) {
        ...
        const response = await this.requestService.post<GetVodUrlParams, Response<VideoUrl>>(this.url.vod(), params);
        if (response.Data.Url) {
            let args = VideoPlayArgs.FromUrl(response.Data.Url);
            if (response.Data.Username) {
                args.username = response.Data.Username;
