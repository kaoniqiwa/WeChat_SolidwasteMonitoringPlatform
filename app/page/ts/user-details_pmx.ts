...
class UserDetails implements IObserver {
  ...
  async createResource(Ids: string[]) {
    ...
    if (this.resourceType == ResourceType.County || this.resourceType == ResourceType.Committees) {
    ...
    this.myAside = new MyAside(this.element.asideContainer, [
      {
        ...
        data: this.resourceList.get(ResourceType[this.resourceType!] as ResourceName),
        type: ResourceType[this.resourceType],
        mode: this.resourceType == ResourceType.GarbageStations ? SelectionMode.multiple : void 0,
        atLeastNum: 1
      },
