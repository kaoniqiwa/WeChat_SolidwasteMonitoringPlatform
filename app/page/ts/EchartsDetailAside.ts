...
import echartDetailAsideTemplate from '../EchartDetailAside.html'
...
export default class EchartAsideDetail extends IAside {
  ...
  dropListTotal: Array<IllegalDropEventRecord | MixedIntoEventRecord | GarbageFullEventRecord | GarbageDropEventRecord> = [];// 拉取到的所有数据
  dropListChunk: Array<IllegalDropEventRecord | MixedIntoEventRecord | GarbageFullEventRecord | GarbageDropEventRecord> = []; // 每次拉取到的数据
  ...
  constructor(selector: HTMLElement | string, private dataController: DataController) {
    ...
    this.elements = {
      ...
      contentContainer: this.innerContainer.querySelector('.card-container') as HTMLDivElement,
      innerBack: this.innerContainer.querySelector('.inner-back') as HTMLDivElement,
      showDatePicker: this.innerContainer.querySelector('#showDatePicker') as HTMLDivElement,
    }
  ...
  async render() {
    ...
    if (this.type == EventType.IllegalDrop || this.type == EventType.MixedInto) {
      ...
      this.myTemplate.bindTo('.echartdetail-inner-container #HistoryDropTemplate');

    } else if (this.type == undefined) {
      this.myTemplate.bindTo('.echartdetail-inner-container #GarbageDropTemplate');
    }
    this.createContent()
  ...
  private async loadData() {
    let data = await this.dataController.GetEventRecordByGarbageStation(this.id, this.currentPage, this.type, this.day)
  ...
  parseData() {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if ('DropImageUrls' in v.Data) {
        v.Data.DropImageUrls && imageUrls.push(...(v.Data.DropImageUrls.map((item) => item.ImageUrl)))
        v.Data.HandleImageUrls && imageUrls.push(...v.Data.HandleImageUrls.map((item) => item.ImageUrl))
        v.Data.TimeoutImageUrls && imageUrls.push(...v.Data.TimeoutImageUrls.map((item) => item.ImageUrl))
      }
